...
module.exports = (
  ...
  const entSubstFn = constant_48cc1015({});
  ...
  return (
    { u1SupFan, uFreProSta, uZonSta, uOutDam_max, TCut, hCut }
  ) => {
    const sub1 = sub1Fn({ u2: TCut });
    ...
    const andEnaDis = andEnaDisFn({ u1: and1.y });
    const not2 = not2Fn({ u: andEnaDis.y });
    const maxOutDam = maxOutDamFn({ u3: uOutDam_max, u2: not2.y });
    ...
    const andFrePro = andFreProFn({ u2: not3.y, y: andEnaDis.u2 });
    const intEqu = intEquFn({ u1: uFreProSta, u2: conInt.y, y: andFrePro.u1 });
